...
//inject:github.com/openai/openai-go/v2
package openai

// Ported from go.opentelemetry.io/otelc instrumentation/github.com/openai/openai-go/v2 (hook.go):
...
func NewClient(opts ...option.RequestOption) (otelcClient Client) {
	if hooksupport.Instrumented("OPENAI") {
		...
		otelcOpts = append(otelcOpts, option.WithMiddleware(genai.OpenAIMiddleware("go.opentelemetry.io/otelc/instrumentation/github.com/openai/openai-go/v2")))
