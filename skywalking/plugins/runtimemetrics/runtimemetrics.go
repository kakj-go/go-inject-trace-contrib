// Package runtimemetrics ports the official skywalking-go runtimemetrics
// plugin. Unlike framework plugins it needs no code injection: it reads the
// public runtime/metrics API and reports instance_golang_* meters, so it is
// a normal package registered through boot.OnTracerReady, which fires right
// after the tracer boots (the official //skywalking:init hook timing).
...
import (
	...
	"github.com/kakj-go/go-inject-trace-contrib/skywalking/boot"
)
...
func init() {
	boot.OnTracerReady(registerMetrics)
}
...
func newMetricsHistogramReplaceInfo(name string, multiples int) *meterInfo {
	return &meterInfo{name: name, isHistogram: true, histogramMultiple: multiples}
}
