...
import (
	...
	_ "github.com/kakj-go/go-inject-trace-contrib/skywalking"
)
...
func TestRaw(db *gorm.DB) error {
	if err := db.Exec(`CREATE TABLE IF NOT EXISTS users (id BIGSERIAL PRIMARY KEY, name TEXT, age INTEGER)`).Error; err != nil {
...
func main() {
	dsn := "host=postgres-server user=postgres password=password dbname=test port=5432 sslmode=disable TimeZone=UTC"
	tmpDB, err := gorm.Open(postgres.Open(dsn), &gorm.Config{PrepareStmt: true})
	if err != nil {
