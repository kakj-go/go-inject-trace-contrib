// MongoDB v1 scenario: connects to the mongo container and runs Ping plus an
// insert/find pair; the injected CommandMonitor (contrib otelmongo) creates a
...
func runMongo() {
	...
	client, err := mongo.Connect(ctx, options.Client().ApplyURI(*mongoU))
	if err != nil {
